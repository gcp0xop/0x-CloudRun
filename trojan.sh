...
LOG_FILE="/tmp/ksgcp_cloudrun_trojan_$(date +%s).log"
touch "$LOG_FILE"
...
clear
printf "\n${C_PURPLE}${BOLD}🚀 KSGCP CLOUD RUN DEPLOYER${RESET} ${C_ORANGE}(Trojan WS Edition)${RESET}\n"
hr
...
IMAGE="docker.io/n4pro/tr:latest"  # Changed to Trojan Image
REGION="us-central1"
CPU="2"
MEMORY="4Gi"
SERVICE="alpha0x1"
...
kv "Region" "${REGION}"
kv "Service" "${SERVICE}"
kv "Protocol" "Trojan WS Only"
kv "Specs" "${CPU} CPU / ${MEMORY} RAM"

# =================== Timezone Setup (5 Hours) ===================
...
# NOTE: HTTP/2 is disabled here for Trojan WS compatibility
run_with_progress "Pushing ${SERVICE} to Cloud Run (HTTP/1.1)" \
...
MSG=$(cat <<EOF
<blockquote>🚀 ALPHA0x1 TROJAN SERVICE</blockquote>
