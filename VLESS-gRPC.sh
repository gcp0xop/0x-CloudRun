...
on_err() {
  ...
  if [[ -t 1 ]]; then printf "\e[?25h"; fi # Show cursor on error
  exit $rc
}
trap on_err ERR
...
echo "  1️⃣ Trojan WS"
echo "  2️⃣ VLESS WS"
echo "  3️⃣ VLESS gRPC  <--- (သင်ဒါကို ရွေးရပါမယ်)"
...
printf "\n${C_GREEN}${BOLD}✨ freegcp0x Deployment Complete — ${CPU}vCPU/${MEMORY} Instance Activated${RESET}\n"
