...
header() {
    clear
    echo -e "${PURPLE}╔════════════════════════════════════════════╗${RESET}"
    echo -e "${PURPLE}║${RESET}       ${RED}${BOLD}🚀 ALPHA${YELLOW}0x1 ${BLUE}POWER SERVER${RESET}           ${PURPLE}║${RESET}"
    echo -e "${PURPLE}╚════════════════════════════════════════════╝${RESET}"
    echo ""
}
...
info "${WHITE}Specs:${RESET}  ${GREEN}8 vCPU / 8GB RAM${RESET}"
info "${WHITE}Limit:${RESET}  ${GREEN}Max Concurrency (1000)${RESET}"

# =================== 4. Deploying ===================
step "Step 3: Deploying Power Node..."

# Enable API quietly
gcloud services enable run.googleapis.com --quiet >/dev/null 2>&1

echo -e "${YELLOW}   🚀 Launching with 8 CPUs / 8GB RAM...${RESET}"
echo "---------------------------------------------------"

# Deploy Command
# 🔥 Max Concurrency is 1000 (Cloud Run Limit)
...
# Telegram Message
MSG="<blockquote>🚀 ${SERVER_NAME} (POWER)</blockquote>
